...
$$('dialog').forEach(d=>d.addEventListener('close',()=>{if(d.id==='manifesto'){setPlaying(false);setSound(false);cardFilms().forEach(v=>{v.pause();v.removeAttribute('src');});cardWanted='';}background();trigger?.focus({preventScroll:true});}));

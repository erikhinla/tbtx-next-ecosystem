...
function resumeWorld(){if(reduced||document.hidden)return;const scene=$('#world')?.dataset.scene;if(!scene||scene==='off')return;if($('#gallery')?.open&&$('#gallery-film')&&!$('#gallery-film').paused)return;videos[slot]?.play().catch(()=>{});$$('.fog-stack video,.route-lane video').forEach(v=>v.play().catch(()=>{}));syncBed(wanted||activeScene?.dataset.world);}document.addEventListener('visibilitychange',()=>{if(document.hidden)$$('video').forEach(v=>v.pause());else resumeWorld();});addEventListener('pageshow',resumeWorld);addEventListener('focus',resumeWorld);

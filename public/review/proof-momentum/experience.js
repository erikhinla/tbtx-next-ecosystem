...
const gateCopy={out:'Consume and be consumed by AI tools without learning them and get passed by those who did.',back:'Continue Managing Digital Fog as AI\'s assistant, always busy but not building something scalable that moves you.',up:'It was never yours to carry. Fifteen questions. See where it shows up.'};
$$('[data-choice]').forEach(b=>b.onclick=()=>{const choice=b.dataset.choice,gateEl=$('#gate .gate')||$('#gate');$$('[data-choice]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));const copy=$('#gate-copy');if(copy){copy.hidden=false;copy.innerHTML=choice==='up'?'It was never yours to carry.<br>Fifteen questions. See where it shows up.':(gateCopy[choice]||'');}gateEl.classList.add('is-answered');setStood(choice==='up');document.body.classList.toggle('held',choice!=='up');if(choice==='up'){document.body.classList.remove('held');const next=$('#routes')?'#routes':($('#build')?'#build':'#proof');go(next);}});
